...
func ValidateStructMultierror(marshalledStruct interface{}, filePath string, structPkgs ...string) (returnError *multierror.Error) {
	...
	if err != nil {
		return multierror.Append(returnError, fmt.Errorf("error loading markers from crd validation: \n\t%v", err))
	}

	typesMap := getAllPackageTypes(pkgs, col)
	if len(typesMap) == 0 {
		returnError = multierror.Append(returnError, fmt.Errorf("%v: loaded classes are empty or not found", filePath))
	}

	returnError = multierror.Append(returnError, validateStruct(marshalledStruct, typesMap, nil, filePath, col))

	return returnError
}

// validateStruct is an internal recursive function that recursively applies markers rules to types and fields
func validateStruct(marshalledStruct interface{}, typesMap map[string]*k8smarkers.TypeInfo, markerValues k8smarkers.MarkerValues, fieldName string, col *k8smarkers.Collector) (returnError error) {
	...
	if unpointedValue.IsValid() && !unpointedValue.IsZero() {
		...
		if markerType != nil {
			...
			returnError = multierror.Append(returnError, applyMarkers(value, markerType.Markers, fieldName, k8smarkers.DescribesType, col))
			...
			for _, field := range markerType.Fields {
				if fieldValue := unpointedValue.FieldByName(field.Name); fieldValue.IsValid() {
					returnError = multierror.Append(returnError, validateStruct(
		...
		if unpointedValue.Kind() == reflect.Slice || unpointedValue.Kind() == reflect.Array {
			for i := 0; i < unpointedValue.Len(); i++ {
				returnError = multierror.Append(returnError, validateStruct(unpointedValue.Index(i).Interface(), typesMap, nil, fieldName+">"+unpointedValue.Type().Name(), col))
			}
		}
	}

	returnError = multierror.Append(returnError, applyMarkers(value, markerValues, fieldName, k8smarkers.DescribesField, col))

	return returnError
}

// applyMarkers applies all markers found in the markers arg to a given type/field
func applyMarkers(value reflect.Value, markers k8smarkers.MarkerValues, fieldName string, targetType k8smarkers.TargetType, col *k8smarkers.Collector) (returnError error) {
	...
	if !reflect.Indirect(value).IsValid() {
		...
		if isRequired != nil {
			...
			if !ok {
				returnError = multierror.Append(returnError, fmt.Errorf("%v: required marker needs to be a bool, check struct definition", fieldName))
			}

			boolIsRequired := bool(typedIsRequired)

			if boolIsRequired {
				returnError = multierror.Append(returnError, fmt.Errorf("%v is required", fieldName))
				return returnError
			}
		}
	...
	for markerName, markerValueList := range markers {
		...
		if thisdef == nil {
			returnError = multierror.Append(returnError, fmt.Errorf("could not find marker definition for %v - check target type", markerName))
			continue
		}
		...
		for _, markerValueInterface := range markerValueList {
			...
			if !isok {
				returnError = multierror.Append(returnError,
			...
			if !ok {
				returnError = multierror.Append(returnError, fmt.Errorf("cannot convert %v to DDmarker, please check the interface definition", thisdef.Output))
			} else {
				// conversions are done, proceed to validation
				err := ddmarker.ApplyRule(value)
				if err != nil {
					returnError = multierror.Append(returnError, fmt.Errorf("%v - %v", fieldName, err))
				}
			}
		}
	}
