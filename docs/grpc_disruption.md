...
<p align="center">
    <kbd>
        <img src="../docs/img/grpc/grpc_fundamentals.png" height=300 width=220 />
    </kbd>
</p>

A computer program can execute a **remote procedure call** (RPC) in a different address space. Remote procedure calls are coded as if they were local procedure calls. gRPC is an open-source general-purpose RPC mechanism for serializing structured data and is used widely in conjunction with microservice architecture. This disruption allows you to apply chaos to upstream and downstream dependencies of microservices.
...
:warning: **At this time, the gRPC disruption is still being BETA-tested.** :warning: 
...
### An application failure may be hard to detect

Consider this gRPC request response pairing of a successful gRPC call:
* A gRPC client makes call to `/chaosdogfood.ChaosDogfood/FoodOrder` over the gRPC connection with message `FoodOrder { animal: "cat" }`.
...
* [I just want to see how this disruption works. How can I get the demo application running locally and apply sample disruptions to it?](/docs/grpc_disruption/demo_instructions.md)
