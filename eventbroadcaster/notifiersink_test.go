	...
	Describe("Create", func() {
		Describe("success cases", func() {
			DescribeTable("when the Notifier is not triggered", func(objectKind string) {
				...
				Entry("a Disruption kind",
					v1beta1.DisruptionKind,
				),
				Entry("a DisruptionCron kind",
			...
			DescribeTable("when the Notifier is triggered", func(event corev1.Event, expectedObject client.Object, expectedNotifType notifTypes.NotificationType) {
				...
				Entry("a Disruption kind event with a warning type",
					corev1.Event{
						InvolvedObject: corev1.ObjectReference{
				...
				Entry("a Disruption kind event with a normal notifiable type",
					corev1.Event{
				...
				Entry("a Disruption kind event with a normal type with a target node recovered reason",
					corev1.Event{
				...
				Entry("a Disruption kind event with a normal type and a Finished reason",
					corev1.Event{
				...
				Entry("a Disruption kind event with a normal type and a DurationOver reason",
					corev1.Event{
				...
				Entry("a Disruption kind event with a normal type and a GCOver reason",
					corev1.Event{
				...
				Entry("a DisruptionCron kind event with a warning type",
					corev1.Event{
						InvolvedObject: corev1.ObjectReference{
				...
				Entry("a DisruptionCron kind event with a normal notifiable type",
					corev1.Event{
						Source: corev1.EventSource{
				...
				Entry("a DisruptionCron kind event with a normal type and a Deleted reason",
					corev1.Event{
		...
		Describe("error cases", func() {
			When("the client fails to get the object", func() {
				DescribeTable("it should return an error", func(objectKind string, expectedErrorMessage string) {
					// Arrange
					expectedError := fmt.Errorf(expectedErrorMessage)
					event := corev1.Event{
						InvolvedObject: corev1.ObjectReference{
							Kind: objectKind,
						},
					...
					mockClient.EXPECT().Get(mock.Anything, mock.Anything, mock.Anything).Return(expectedError)
					...
					Entry("with a Disruption kind event",
						v1beta1.DisruptionKind,
						"eventnotifier: unable to get Disruption object",
					),
					Entry("with a DisruptionCron kind event",
						v1beta1.DisruptionCronKind,
						"eventnotifier: unable to get DisruptionCron object",
					),
				)
			})
