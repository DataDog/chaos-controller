...
const (
	...
	metricPrefixNetworkFailure = "chaos.controller.nfi"
)

// NetworkFailureInjectionReconciler reconciles a NetworkFailureInjection object
type NetworkFailureInjectionReconciler struct {
	client.Client
	Log      logr.Logger
...
// Reconcile loop
func (r *NetworkFailureInjectionReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	_ = context.Background()
	_ = r.Log.WithValues("networkfailureinjection", req.NamespacedName)
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if containsString(instance.ObjectMeta.Finalizers, cleanupFinalizer) {
			...
			r.Log.Info("Checking status of cleanup pods before deleting nfi", "numcleanuppods", len(cleanupPods), "networkfailureinjection", instance.Name, "namespace", instance.Namespace)
			...
			for _, cleanupPod := range cleanupPods {
				if cleanupPod.Status.Phase != corev1.PodSucceeded {
					r.Log.Info("Cleanup pod has not completed, retrying nfi deletion", "networkfailureinjection", instance.Name, "namespace", instance.Namespace, "cleanuppod", cleanupPod.Name, "phase", cleanupPod.Status.Phase)
					return ctrl.Result{
						Requeue: true,
					}, nil
				}
	...
	for _, p := range pods.Items {
		...
		if err != nil && errors.IsNotFound(err) {
			...
			r.Log.Info("Creating chaos pod", "networkfailureinjection", instance.Name, "namespace", pod.Namespace, "name", pod.Name, "nodename", nodeName)
			...
			datadog.EventNewPod("New Injected NetworkFailureInjection Pod", fmt.Sprintf("Created failure injection pod for networkfailureinjection \"%s\"", instance.Name), instance.Spec.Failure.Host, []string{"phase:inject", "target_pod:" + p.ObjectMeta.Name, "name:" + instance.Name, "namespace:" + instance.Namespace})
...
func (r *NetworkFailureInjectionReconciler) getPodsToCleanup(instance *chaosv1beta1.NetworkFailureInjection) ([]*corev1.Pod, error) {
	...
	for _, podName := range instance.Status.Pods {
		...
		if errors.IsNotFound(err) {
			r.Log.Info("Cleanup: Pod no longer exists", "networkfailureinjection", instance.Name, "namespace", instance.Namespace, "name", podName, "numpodstotarget", instance.Spec.NumPodsToTarget)
			continue
		} else if err != nil {
			return nil, err
		}
...
func (r *NetworkFailureInjectionReconciler) cleanFailures(instance *chaosv1beta1.NetworkFailureInjection) error {
	...
	for _, p := range pods {
		...
		datadog.EventNewPod("New Cleanup NetworkFailureInjection Pod", fmt.Sprintf("Created cleanup pod for networkfailureinjection \"%s\"", instance.Name), instance.Spec.Failure.Host, []string{"phase:inject", "target_pod:" + p.ObjectMeta.Name, "name:" + instance.Name, "namespace:" + instance.Namespace})
...
func (r *NetworkFailureInjectionReconciler) updateStatusPods(instance *chaosv1beta1.NetworkFailureInjection, pods *corev1.PodList) error {
	...
	r.Log.Info("Updated nfi status with pods selected for injection", "networkfailureinjection", instance.Name, "namespace", instance.Namespace, "numpodstotarget", instance.Spec.NumPodsToTarget)
...
func (r *NetworkFailureInjectionReconciler) selectPodsForInjection(instance *chaosv1beta1.NetworkFailureInjection) (*corev1.PodList, error) {
	...
	for i := 0; i < numPodsToSelect; i++ {
		...
		r.Log.Info("Selected random pod", "name", chosenPod.Name, "namespace", chosenPod.Namespace, "networkfailureinjection", instance.Name, "numpodstotarget", instance.Spec.NumPodsToTarget)
...
func (r *NetworkFailureInjectionReconciler) getCleanupPods(instance *chaosv1beta1.NetworkFailureInjection) ([]corev1.Pod, error) {
...
func (r *NetworkFailureInjectionReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&chaosv1beta1.NetworkFailureInjection{}).
		Complete(r)
}
