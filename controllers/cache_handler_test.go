...
var _ = Describe("Cache Handler verifications", func() {
	...
	Context("events sent verification", func() {
		...
		It("should not fire any warning event on disruption", func() {
			...
			event := findEvent(disruption.CreationTimestamp.Time, v1beta1.EventDisrupted, eventList.Items, targetPodA.Name)
			Expect(event).ToNot(BeNil())

			By("ensuring container target in warning state event was not fired")
			event = findEvent(disruption.CreationTimestamp.Time, v1beta1.EventContainerWarningState, eventList.Items, targetPodA.Name)
