...
type DisruptionCronEvent struct {
	DisruptionCronName string `jsonapi:"attribute" json:"disruption_cron_name"`
	DisruptionCron     string `jsonapi:"attribute" json:"disruption_cron"`
}
...
func (n *Notifier) notifyDisruption(dis v1beta1.Disruption, event corev1.Event, notifType types.NotificationType) error {
	...
	notifierEvent.DisruptionEvent = &DisruptionEvent{
	...
	return n.emitEvent(jsonNotif, logger)
}
...
func (n *Notifier) notifyDisruptionCron(disruptionCron v1beta1.DisruptionCron, event corev1.Event, notifType types.NotificationType) error {
	...
	notifierEvent.DisruptionCronEvent = &DisruptionCronEvent{
		DisruptionCronName: disruptionCron.Name,
		DisruptionCron:     string(disruptionCronStr),
	}
...
func (n *Notifier) emitEvent(jsonNotif []byte, logger *zap.SugaredLogger) error {
