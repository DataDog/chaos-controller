...
type BackgroundProcessManager interface {
	// Start will create a command, wait for it's successful start and notify associated process regularly by sending a SIGCONT signal
	Start(disruptionArgs chaosapi.DisruptionArgs, targetContainer string, args []string) (BackgroundProcess, error)
}

// BackgroundProcess enable to perform actions on a background process
type BackgroundProcess interface {
	// Stop halt any keepAlive associated with background process and related command execution
	Stop()
}
...
func NewBackgroundProcessManager(log *zap.SugaredLogger, processManager Manager, deadline time.Time) BackgroundProcessManager {
...
func (b backgroundProcessManager) Start(disruptionArgs chaosapi.DisruptionArgs, targetContainer string, args []string) (BackgroundProcess, error) {
	log := b.log.With("targetContainer", targetContainer)

	targetContainerID, ok := disruptionArgs.TargetContainers[targetContainer]
	if !ok {
		return nil, fmt.Errorf("targeted container does not exists: %s", targetContainer)
	}

	disruptionArgs.TargetContainers = map[string]string{targetContainer: targetContainerID}

	args = append(args, "--parent-pid", strconv.Itoa(b.processManager.ProcessID()), "--deadline", b.deadline.Format(time.RFC3339))
	args = chaosapi.AppendArgs(args, disruptionArgs)
	...
	go func() {
		for {
			...
			case <-d.ticker.C:
				...
				if err != nil {
					d.log.Errorw("an error occurred when trying to Find process, stopping background process", "err", err)
					d.Stop()
				...
				if err := d.processManager.Signal(process, syscall.SIGCONT); err != nil {
					d.log.Errorw("an error occurred when sending SIGCONT signal to process, stopping background process", "err", err)
					d.Stop()
...
func (d *backgroundProcess) Stop() {
	...
	if err != nil {
		d.log.Warnw("an error occurred while sending interrupt signal to process, cleaning ignored", "err", err)
		return
	}
