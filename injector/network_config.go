...
// AddDrop adds a network probability drop disruption using the drivers in the NetworkDisruptionConfigStruct
func (c NetworkDisruptionConfigStruct) AddDrop(hosts []string, port int, drop int) {
	// closure which adds drop rate
	operation := func(link network.NetlinkLink, parent string) error {
		return c.TrafficController.AddDrop(link.Name(), parent, 0, drop)
	}

	c.addOperation(hosts, port, operation)
}

// AddCorrupt adds a network corruption disruption using the drivers in the NetworkDisruptionConfigStruct
func (c NetworkDisruptionConfigStruct) AddCorrupt(hosts []string, port int, corrupt int) {
	// closure which adds corruption
	operation := func(link network.NetlinkLink, parent string) error {
		return c.TrafficController.AddCorrupt(link.Name(), parent, 0, corrupt)
	}
