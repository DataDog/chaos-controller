...
func (i *networkDisruptionInjector) applyOperations() error {
	...
	if len(i.spec.Hosts) == 0 && len(i.spec.Services) == 0 && len(i.spec.Pods) == 0 {
		...
		i.addFiltersForPods(interfaces, "1:4")
	}
...
func (i *networkDisruptionInjector) addFiltersForServices(interfaces []string, flowid string) error {
	for _, serviceSpec := range i.spec.Services {
		...
		go i.watchChanges(
			...
			PodWatcher{ // build the watcher for the pods related to the service we're watching
				namespace:      serviceSpec.Namespace,
				ports:          NewServicePorts(k8sService.Spec.Ports),
				selector:       labels.SelectorFromValidatedSet(k8sService.Spec.Selector).String(),
				eventChannel:        nil,
				tcFilters:      []TcFilter{},
...
// addFiltersForServices creates tc filters on given interfaces for services in disruption spec classifying matching packets in the given flowid
func (i *networkDisruptionInjector) addFiltersForPods(interfaces []string, flowid string) {
	for _, podSpec := range i.spec.Pods {
		go i.watchChanges(
			ServiceWatcher{ // to be able to use the same function everywhere, we need to declare watchers as dummies
				isDummy: true,
			},
			PodWatcher{
				namespace:      "",
				ports:          []SimplifiedServicePort{{port: podSpec.Port, protocol: podSpec.Protocol}},
				selector:       podSpec.Selector.String(),
				eventChannel:        nil,
				tcFilters:      []TcFilter{},
				podsWithoutIPs: []string{},
			},
