// Code generated by mockery. DO NOT EDIT.

// Unless explicitly stated otherwise all files in this repository are licensed
// under the Apache License Version 2.0.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2026 Datadog, Inc.

package mocks

import (
	cache "sigs.k8s.io/controller-runtime/pkg/cache"
	client "sigs.k8s.io/controller-runtime/pkg/client"

	config "sigs.k8s.io/controller-runtime/pkg/config"

	context "context"

	healthz "sigs.k8s.io/controller-runtime/pkg/healthz"

	http "net/http"

	logr "github.com/go-logr/logr"

	manager "sigs.k8s.io/controller-runtime/pkg/manager"

	meta "k8s.io/apimachinery/pkg/api/meta"

	mock "github.com/stretchr/testify/mock"

	record "k8s.io/client-go/tools/record"

	rest "k8s.io/client-go/rest"

	runtime "k8s.io/apimachinery/pkg/runtime"

	events "k8s.io/client-go/tools/events"
	webhook "sigs.k8s.io/controller-runtime/pkg/webhook"
	conversion "sigs.k8s.io/controller-runtime/pkg/webhook/conversion"
)

// ManagerMock is an autogenerated mock type for the Manager type
type ManagerMock struct {
	mock.Mock
}

type ManagerMock_Expecter struct {
	mock *mock.Mock
}

func (_m *ManagerMock) EXPECT() *ManagerMock_Expecter {
	return &ManagerMock_Expecter{mock: &_m.Mock}
}

// Add provides a mock function with given fields: _a0
func (_m *ManagerMock) Add(_a0 manager.Runnable) error {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for Add")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(manager.Runnable) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ManagerMock_Add_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Add'
type ManagerMock_Add_Call struct {
	*mock.Call
}

// Add is a helper method to define mock.On call
//   - _a0 manager.Runnable
func (_e *ManagerMock_Expecter) Add(_a0 interface{}) *ManagerMock_Add_Call {
	return &ManagerMock_Add_Call{Call: _e.mock.On("Add", _a0)}
}

func (_c *ManagerMock_Add_Call) Run(run func(_a0 manager.Runnable)) *ManagerMock_Add_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(manager.Runnable))
	})
	return _c
}

func (_c *ManagerMock_Add_Call) Return(_a0 error) *ManagerMock_Add_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_Add_Call) RunAndReturn(run func(manager.Runnable) error) *ManagerMock_Add_Call {
	_c.Call.Return(run)
	return _c
}

// AddHealthzCheck provides a mock function with given fields: name, check
func (_m *ManagerMock) AddHealthzCheck(name string, check healthz.Checker) error {
	ret := _m.Called(name, check)

	if len(ret) == 0 {
		panic("no return value specified for AddHealthzCheck")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(string, healthz.Checker) error); ok {
		r0 = rf(name, check)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ManagerMock_AddHealthzCheck_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'AddHealthzCheck'
type ManagerMock_AddHealthzCheck_Call struct {
	*mock.Call
}

// AddHealthzCheck is a helper method to define mock.On call
//   - name string
//   - check healthz.Checker
func (_e *ManagerMock_Expecter) AddHealthzCheck(name interface{}, check interface{}) *ManagerMock_AddHealthzCheck_Call {
	return &ManagerMock_AddHealthzCheck_Call{Call: _e.mock.On("AddHealthzCheck", name, check)}
}

func (_c *ManagerMock_AddHealthzCheck_Call) Run(run func(name string, check healthz.Checker)) *ManagerMock_AddHealthzCheck_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(healthz.Checker))
	})
	return _c
}

func (_c *ManagerMock_AddHealthzCheck_Call) Return(_a0 error) *ManagerMock_AddHealthzCheck_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_AddHealthzCheck_Call) RunAndReturn(run func(string, healthz.Checker) error) *ManagerMock_AddHealthzCheck_Call {
	_c.Call.Return(run)
	return _c
}

// AddMetricsServerExtraHandler provides a mock function with given fields: path, handler
func (_m *ManagerMock) AddMetricsServerExtraHandler(path string, handler http.Handler) error {
	ret := _m.Called(path, handler)

	if len(ret) == 0 {
		panic("no return value specified for AddMetricsServerExtraHandler")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(string, http.Handler) error); ok {
		r0 = rf(path, handler)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ManagerMock_AddMetricsServerExtraHandler_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'AddMetricsServerExtraHandler'
type ManagerMock_AddMetricsServerExtraHandler_Call struct {
	*mock.Call
}

// AddMetricsServerExtraHandler is a helper method to define mock.On call
//   - path string
//   - handler http.Handler
func (_e *ManagerMock_Expecter) AddMetricsServerExtraHandler(path interface{}, handler interface{}) *ManagerMock_AddMetricsServerExtraHandler_Call {
	return &ManagerMock_AddMetricsServerExtraHandler_Call{Call: _e.mock.On("AddMetricsServerExtraHandler", path, handler)}
}

func (_c *ManagerMock_AddMetricsServerExtraHandler_Call) Run(run func(path string, handler http.Handler)) *ManagerMock_AddMetricsServerExtraHandler_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(http.Handler))
	})
	return _c
}

func (_c *ManagerMock_AddMetricsServerExtraHandler_Call) Return(_a0 error) *ManagerMock_AddMetricsServerExtraHandler_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_AddMetricsServerExtraHandler_Call) RunAndReturn(run func(string, http.Handler) error) *ManagerMock_AddMetricsServerExtraHandler_Call {
	_c.Call.Return(run)
	return _c
}

// AddReadyzCheck provides a mock function with given fields: name, check
func (_m *ManagerMock) AddReadyzCheck(name string, check healthz.Checker) error {
	ret := _m.Called(name, check)

	if len(ret) == 0 {
		panic("no return value specified for AddReadyzCheck")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(string, healthz.Checker) error); ok {
		r0 = rf(name, check)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ManagerMock_AddReadyzCheck_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'AddReadyzCheck'
type ManagerMock_AddReadyzCheck_Call struct {
	*mock.Call
}

// AddReadyzCheck is a helper method to define mock.On call
//   - name string
//   - check healthz.Checker
func (_e *ManagerMock_Expecter) AddReadyzCheck(name interface{}, check interface{}) *ManagerMock_AddReadyzCheck_Call {
	return &ManagerMock_AddReadyzCheck_Call{Call: _e.mock.On("AddReadyzCheck", name, check)}
}

func (_c *ManagerMock_AddReadyzCheck_Call) Run(run func(name string, check healthz.Checker)) *ManagerMock_AddReadyzCheck_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(healthz.Checker))
	})
	return _c
}

func (_c *ManagerMock_AddReadyzCheck_Call) Return(_a0 error) *ManagerMock_AddReadyzCheck_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_AddReadyzCheck_Call) RunAndReturn(run func(string, healthz.Checker) error) *ManagerMock_AddReadyzCheck_Call {
	_c.Call.Return(run)
	return _c
}

// Elected provides a mock function with no fields
func (_m *ManagerMock) Elected() <-chan struct{} {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for Elected")
	}

	var r0 <-chan struct{}
	if rf, ok := ret.Get(0).(func() <-chan struct{}); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(<-chan struct{})
		}
	}

	return r0
}

// ManagerMock_Elected_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Elected'
type ManagerMock_Elected_Call struct {
	*mock.Call
}

// Elected is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) Elected() *ManagerMock_Elected_Call {
	return &ManagerMock_Elected_Call{Call: _e.mock.On("Elected")}
}

func (_c *ManagerMock_Elected_Call) Run(run func()) *ManagerMock_Elected_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_Elected_Call) Return(_a0 <-chan struct{}) *ManagerMock_Elected_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_Elected_Call) RunAndReturn(run func() <-chan struct{}) *ManagerMock_Elected_Call {
	_c.Call.Return(run)
	return _c
}

// GetAPIReader provides a mock function with no fields
func (_m *ManagerMock) GetAPIReader() client.Reader {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetAPIReader")
	}

	var r0 client.Reader
	if rf, ok := ret.Get(0).(func() client.Reader); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(client.Reader)
		}
	}

	return r0
}

// ManagerMock_GetAPIReader_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetAPIReader'
type ManagerMock_GetAPIReader_Call struct {
	*mock.Call
}

// GetAPIReader is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetAPIReader() *ManagerMock_GetAPIReader_Call {
	return &ManagerMock_GetAPIReader_Call{Call: _e.mock.On("GetAPIReader")}
}

func (_c *ManagerMock_GetAPIReader_Call) Run(run func()) *ManagerMock_GetAPIReader_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetAPIReader_Call) Return(_a0 client.Reader) *ManagerMock_GetAPIReader_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetAPIReader_Call) RunAndReturn(run func() client.Reader) *ManagerMock_GetAPIReader_Call {
	_c.Call.Return(run)
	return _c
}

// GetCache provides a mock function with no fields
func (_m *ManagerMock) GetCache() cache.Cache {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetCache")
	}

	var r0 cache.Cache
	if rf, ok := ret.Get(0).(func() cache.Cache); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(cache.Cache)
		}
	}

	return r0
}

// ManagerMock_GetCache_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetCache'
type ManagerMock_GetCache_Call struct {
	*mock.Call
}

// GetCache is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetCache() *ManagerMock_GetCache_Call {
	return &ManagerMock_GetCache_Call{Call: _e.mock.On("GetCache")}
}

func (_c *ManagerMock_GetCache_Call) Run(run func()) *ManagerMock_GetCache_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetCache_Call) Return(_a0 cache.Cache) *ManagerMock_GetCache_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetCache_Call) RunAndReturn(run func() cache.Cache) *ManagerMock_GetCache_Call {
	_c.Call.Return(run)
	return _c
}

// GetClient provides a mock function with no fields
func (_m *ManagerMock) GetClient() client.Client {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetClient")
	}

	var r0 client.Client
	if rf, ok := ret.Get(0).(func() client.Client); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(client.Client)
		}
	}

	return r0
}

// ManagerMock_GetClient_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetClient'
type ManagerMock_GetClient_Call struct {
	*mock.Call
}

// GetClient is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetClient() *ManagerMock_GetClient_Call {
	return &ManagerMock_GetClient_Call{Call: _e.mock.On("GetClient")}
}

func (_c *ManagerMock_GetClient_Call) Run(run func()) *ManagerMock_GetClient_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetClient_Call) Return(_a0 client.Client) *ManagerMock_GetClient_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetClient_Call) RunAndReturn(run func() client.Client) *ManagerMock_GetClient_Call {
	_c.Call.Return(run)
	return _c
}

// GetConfig provides a mock function with no fields
func (_m *ManagerMock) GetConfig() *rest.Config {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetConfig")
	}

	var r0 *rest.Config
	if rf, ok := ret.Get(0).(func() *rest.Config); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*rest.Config)
		}
	}

	return r0
}

// ManagerMock_GetConfig_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetConfig'
type ManagerMock_GetConfig_Call struct {
	*mock.Call
}

// GetConfig is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetConfig() *ManagerMock_GetConfig_Call {
	return &ManagerMock_GetConfig_Call{Call: _e.mock.On("GetConfig")}
}

func (_c *ManagerMock_GetConfig_Call) Run(run func()) *ManagerMock_GetConfig_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetConfig_Call) Return(_a0 *rest.Config) *ManagerMock_GetConfig_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetConfig_Call) RunAndReturn(run func() *rest.Config) *ManagerMock_GetConfig_Call {
	_c.Call.Return(run)
	return _c
}

// GetControllerOptions provides a mock function with no fields
func (_m *ManagerMock) GetControllerOptions() config.Controller {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetControllerOptions")
	}

	var r0 config.Controller
	if rf, ok := ret.Get(0).(func() config.Controller); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(config.Controller)
	}

	return r0
}

// ManagerMock_GetControllerOptions_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetControllerOptions'
type ManagerMock_GetControllerOptions_Call struct {
	*mock.Call
}

// GetControllerOptions is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetControllerOptions() *ManagerMock_GetControllerOptions_Call {
	return &ManagerMock_GetControllerOptions_Call{Call: _e.mock.On("GetControllerOptions")}
}

func (_c *ManagerMock_GetControllerOptions_Call) Run(run func()) *ManagerMock_GetControllerOptions_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetControllerOptions_Call) Return(_a0 config.Controller) *ManagerMock_GetControllerOptions_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetControllerOptions_Call) RunAndReturn(run func() config.Controller) *ManagerMock_GetControllerOptions_Call {
	_c.Call.Return(run)
	return _c
}

// GetEventRecorderFor provides a mock function with given fields: name
func (_m *ManagerMock) GetEventRecorderFor(name string) record.EventRecorder {
	ret := _m.Called(name)

	if len(ret) == 0 {
		panic("no return value specified for GetEventRecorderFor")
	}

	var r0 record.EventRecorder
	if rf, ok := ret.Get(0).(func(string) record.EventRecorder); ok {
		r0 = rf(name)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(record.EventRecorder)
		}
	}

	return r0
}

// ManagerMock_GetEventRecorderFor_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetEventRecorderFor'
type ManagerMock_GetEventRecorderFor_Call struct {
	*mock.Call
}

// GetEventRecorderFor is a helper method to define mock.On call
//   - name string
func (_e *ManagerMock_Expecter) GetEventRecorderFor(name interface{}) *ManagerMock_GetEventRecorderFor_Call {
	return &ManagerMock_GetEventRecorderFor_Call{Call: _e.mock.On("GetEventRecorderFor", name)}
}

func (_c *ManagerMock_GetEventRecorderFor_Call) Run(run func(name string)) *ManagerMock_GetEventRecorderFor_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string))
	})
	return _c
}

func (_c *ManagerMock_GetEventRecorderFor_Call) Return(_a0 record.EventRecorder) *ManagerMock_GetEventRecorderFor_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetEventRecorderFor_Call) RunAndReturn(run func(string) record.EventRecorder) *ManagerMock_GetEventRecorderFor_Call {
	_c.Call.Return(run)
	return _c
}

// GetEventRecorder provides a mock function with given fields: name
func (_m *ManagerMock) GetEventRecorder(name string) events.EventRecorder {
	ret := _m.Called(name)

	if len(ret) == 0 {
		panic("no return value specified for GetEventRecorder")
	}

	var r0 events.EventRecorder
	if rf, ok := ret.Get(0).(func(string) events.EventRecorder); ok {
		r0 = rf(name)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(events.EventRecorder)
		}
	}

	return r0
}

// ManagerMock_GetEventRecorder_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetEventRecorder'
type ManagerMock_GetEventRecorder_Call struct {
	*mock.Call
}

// GetEventRecorder is a helper method to define mock.On call
//   - name string
func (_e *ManagerMock_Expecter) GetEventRecorder(name interface{}) *ManagerMock_GetEventRecorder_Call {
	return &ManagerMock_GetEventRecorder_Call{Call: _e.mock.On("GetEventRecorder", name)}
}

func (_c *ManagerMock_GetEventRecorder_Call) Run(run func(name string)) *ManagerMock_GetEventRecorder_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string))
	})
	return _c
}

func (_c *ManagerMock_GetEventRecorder_Call) Return(_a0 events.EventRecorder) *ManagerMock_GetEventRecorder_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetEventRecorder_Call) RunAndReturn(run func(string) events.EventRecorder) *ManagerMock_GetEventRecorder_Call {
	_c.Call.Return(run)
	return _c
}

// GetFieldIndexer provides a mock function with no fields
func (_m *ManagerMock) GetFieldIndexer() client.FieldIndexer {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetFieldIndexer")
	}

	var r0 client.FieldIndexer
	if rf, ok := ret.Get(0).(func() client.FieldIndexer); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(client.FieldIndexer)
		}
	}

	return r0
}

// ManagerMock_GetFieldIndexer_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetFieldIndexer'
type ManagerMock_GetFieldIndexer_Call struct {
	*mock.Call
}

// GetFieldIndexer is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetFieldIndexer() *ManagerMock_GetFieldIndexer_Call {
	return &ManagerMock_GetFieldIndexer_Call{Call: _e.mock.On("GetFieldIndexer")}
}

func (_c *ManagerMock_GetFieldIndexer_Call) Run(run func()) *ManagerMock_GetFieldIndexer_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetFieldIndexer_Call) Return(_a0 client.FieldIndexer) *ManagerMock_GetFieldIndexer_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetFieldIndexer_Call) RunAndReturn(run func() client.FieldIndexer) *ManagerMock_GetFieldIndexer_Call {
	_c.Call.Return(run)
	return _c
}

// GetHTTPClient provides a mock function with no fields
func (_m *ManagerMock) GetHTTPClient() *http.Client {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetHTTPClient")
	}

	var r0 *http.Client
	if rf, ok := ret.Get(0).(func() *http.Client); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*http.Client)
		}
	}

	return r0
}

// ManagerMock_GetHTTPClient_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetHTTPClient'
type ManagerMock_GetHTTPClient_Call struct {
	*mock.Call
}

// GetHTTPClient is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetHTTPClient() *ManagerMock_GetHTTPClient_Call {
	return &ManagerMock_GetHTTPClient_Call{Call: _e.mock.On("GetHTTPClient")}
}

func (_c *ManagerMock_GetHTTPClient_Call) Run(run func()) *ManagerMock_GetHTTPClient_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetHTTPClient_Call) Return(_a0 *http.Client) *ManagerMock_GetHTTPClient_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetHTTPClient_Call) RunAndReturn(run func() *http.Client) *ManagerMock_GetHTTPClient_Call {
	_c.Call.Return(run)
	return _c
}

// GetLogger provides a mock function with no fields
func (_m *ManagerMock) GetLogger() logr.Logger {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetLogger")
	}

	var r0 logr.Logger
	if rf, ok := ret.Get(0).(func() logr.Logger); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(logr.Logger)
	}

	return r0
}

// ManagerMock_GetLogger_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetLogger'
type ManagerMock_GetLogger_Call struct {
	*mock.Call
}

// GetLogger is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetLogger() *ManagerMock_GetLogger_Call {
	return &ManagerMock_GetLogger_Call{Call: _e.mock.On("GetLogger")}
}

func (_c *ManagerMock_GetLogger_Call) Run(run func()) *ManagerMock_GetLogger_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetLogger_Call) Return(_a0 logr.Logger) *ManagerMock_GetLogger_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetLogger_Call) RunAndReturn(run func() logr.Logger) *ManagerMock_GetLogger_Call {
	_c.Call.Return(run)
	return _c
}

// GetRESTMapper provides a mock function with no fields
func (_m *ManagerMock) GetRESTMapper() meta.RESTMapper {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetRESTMapper")
	}

	var r0 meta.RESTMapper
	if rf, ok := ret.Get(0).(func() meta.RESTMapper); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(meta.RESTMapper)
		}
	}

	return r0
}

// ManagerMock_GetRESTMapper_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetRESTMapper'
type ManagerMock_GetRESTMapper_Call struct {
	*mock.Call
}

// GetRESTMapper is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetRESTMapper() *ManagerMock_GetRESTMapper_Call {
	return &ManagerMock_GetRESTMapper_Call{Call: _e.mock.On("GetRESTMapper")}
}

func (_c *ManagerMock_GetRESTMapper_Call) Run(run func()) *ManagerMock_GetRESTMapper_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetRESTMapper_Call) Return(_a0 meta.RESTMapper) *ManagerMock_GetRESTMapper_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetRESTMapper_Call) RunAndReturn(run func() meta.RESTMapper) *ManagerMock_GetRESTMapper_Call {
	_c.Call.Return(run)
	return _c
}

// GetScheme provides a mock function with no fields
func (_m *ManagerMock) GetScheme() *runtime.Scheme {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetScheme")
	}

	var r0 *runtime.Scheme
	if rf, ok := ret.Get(0).(func() *runtime.Scheme); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*runtime.Scheme)
		}
	}

	return r0
}

// ManagerMock_GetScheme_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetScheme'
type ManagerMock_GetScheme_Call struct {
	*mock.Call
}

// GetScheme is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetScheme() *ManagerMock_GetScheme_Call {
	return &ManagerMock_GetScheme_Call{Call: _e.mock.On("GetScheme")}
}

func (_c *ManagerMock_GetScheme_Call) Run(run func()) *ManagerMock_GetScheme_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetScheme_Call) Return(_a0 *runtime.Scheme) *ManagerMock_GetScheme_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetScheme_Call) RunAndReturn(run func() *runtime.Scheme) *ManagerMock_GetScheme_Call {
	_c.Call.Return(run)
	return _c
}

// GetWebhookServer provides a mock function with no fields
func (_m *ManagerMock) GetWebhookServer() webhook.Server {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetWebhookServer")
	}

	var r0 webhook.Server
	if rf, ok := ret.Get(0).(func() webhook.Server); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(webhook.Server)
		}
	}

	return r0
}

// ManagerMock_GetWebhookServer_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetWebhookServer'
type ManagerMock_GetWebhookServer_Call struct {
	*mock.Call
}

// GetWebhookServer is a helper method to define mock.On call
func (_e *ManagerMock_Expecter) GetWebhookServer() *ManagerMock_GetWebhookServer_Call {
	return &ManagerMock_GetWebhookServer_Call{Call: _e.mock.On("GetWebhookServer")}
}

func (_c *ManagerMock_GetWebhookServer_Call) Run(run func()) *ManagerMock_GetWebhookServer_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetWebhookServer_Call) Return(_a0 webhook.Server) *ManagerMock_GetWebhookServer_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetWebhookServer_Call) RunAndReturn(run func() webhook.Server) *ManagerMock_GetWebhookServer_Call {
	_c.Call.Return(run)
	return _c
}

// Start provides a mock function with given fields: ctx
func (_m *ManagerMock) Start(ctx context.Context) error {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for Start")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context) error); ok {
		r0 = rf(ctx)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// ManagerMock_Start_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Start'
type ManagerMock_Start_Call struct {
	*mock.Call
}

// Start is a helper method to define mock.On call
//   - ctx context.Context
func (_e *ManagerMock_Expecter) Start(ctx interface{}) *ManagerMock_Start_Call {
	return &ManagerMock_Start_Call{Call: _e.mock.On("Start", ctx)}
}

func (_c *ManagerMock_Start_Call) Run(run func(ctx context.Context)) *ManagerMock_Start_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context))
	})
	return _c
}

func (_c *ManagerMock_Start_Call) Return(_a0 error) *ManagerMock_Start_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_Start_Call) RunAndReturn(run func(context.Context) error) *ManagerMock_Start_Call {
	_c.Call.Return(run)
	return _c
}

// GetConverterRegistry provides a mock function with no given fields.
func (_m *ManagerMock) GetConverterRegistry() conversion.Registry {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetConverterRegistry")
	}

	var r0 conversion.Registry
	if rf, ok := ret.Get(0).(func() conversion.Registry); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(conversion.Registry)
		}
	}

	return r0
}

// ManagerMock_GetConverterRegistry_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetConverterRegistry'
type ManagerMock_GetConverterRegistry_Call struct {
	*mock.Call
}

func (_e *ManagerMock_Expecter) GetConverterRegistry() *ManagerMock_GetConverterRegistry_Call {
	return &ManagerMock_GetConverterRegistry_Call{Call: _e.mock.On("GetConverterRegistry")}
}

func (_c *ManagerMock_GetConverterRegistry_Call) Run(run func()) *ManagerMock_GetConverterRegistry_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *ManagerMock_GetConverterRegistry_Call) Return(_a0 conversion.Registry) *ManagerMock_GetConverterRegistry_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *ManagerMock_GetConverterRegistry_Call) RunAndReturn(run func() conversion.Registry) *ManagerMock_GetConverterRegistry_Call {
	_c.Call.Return(run)
	return _c
}

// NewManagerMock creates a new instance of ManagerMock. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewManagerMock(t interface {
	mock.TestingT
	Cleanup(func())
}) *ManagerMock {
	mock := &ManagerMock{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
