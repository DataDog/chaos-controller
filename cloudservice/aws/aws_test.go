...
var _ = Describe("AWS Parsing", func() {
	Context("Parse AWS IP Range file", func() {
		...
		version, ipRanges, err := awsManager.ConvertToGenericIPRanges([]byte(ipRangeFile))
		...
		It("should parse the ip range file", func() {
			...
			Expect(len(ipRanges["AMAZON"])).To(Equal(0))
			Expect(len(ipRanges["S3"])).To(Equal(1))
		})
		...
		It("should error on parsing", func() {
			...
			_, _, err := awsManager.ConvertToGenericIPRanges([]byte(ipRangeFile))
