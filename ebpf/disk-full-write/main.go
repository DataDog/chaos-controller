...
var nExitCode = flag.Uint64("exit-code", 28, "Exit code (default ENOSPC=28)")
...
func main() {
	...
	bpfModule, err := bpf.NewModuleFromFile("/usr/local/bin/bpf-disk-full-write.bpf.o")
	must(err)
	...
	prog, err := bpfModule.GetProgram("injection_disk_full_write")
	must(err)

	// Attach the kprobe to catch sys_write syscall
	_, err = prog.AttachKprobe(ebpf.SysWrite)
	must(err)
...
func printEvent(data []byte) {
	...
	logger.Infof("Disrupt write Ppid %d, Pid %d, Tid: %d, Gid: %d, Command: %s", ppid, pid, tid, gid, comm)
}

func initGlobalVariables(bpfModule *bpf.Module) {
	flag.Parse()

	var pid uint32 = uint32(*nPid)
	if err := bpfModule.InitGlobalVariable("target_pid", pid); err != nil {
		must(err)
	}

	var exitCode uint32 = uint32(*nExitCode)
	if err := bpfModule.InitGlobalVariable("exit_code", exitCode); err != nil {
		must(err)
	}

	var probability uint32 = uint32(*nProbability)
