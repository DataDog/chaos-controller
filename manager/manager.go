...
func StartManager(tlsConfig *tls.Config) {
	...
	logger.Infow("DEBUG", "certDir", cfg.Controller.Webhook.CertDir, "tlsConfig", tlsConfig)

	if tlsConfig != nil {
		tlsFunc := func(c *tls.Config) {
			c.GetCertificate = tlsConfig.GetCertificate
			c.GetClientCertificate = tlsConfig.GetClientCertificate
			c.GetConfigForClient = tlsConfig.GetConfigForClient
			...
			c.ClientAuth = tlsConfig.ClientAuth
			c.ClientCAs = tlsConfig.ClientCAs
			c.InsecureSkipVerify = tlsConfig.InsecureSkipVerify
			c.CipherSuites = tlsConfig.CipherSuites
			c.ClientSessionCache = tlsConfig.ClientSessionCache
			c.UnwrapSession = tlsConfig.UnwrapSession
			c.WrapSession = tlsConfig.WrapSession
			c.MinVersion = tlsConfig.MinVersion
			c.MaxVersion = tlsConfig.MaxVersion
			c.CurvePreferences = tlsConfig.CurvePreferences
			c.Renegotiation = tlsConfig.Renegotiation
		}

		tlsOpts := []func(*tls.Config){tlsFunc}
		webhookOpts.TLSOpts = tlsOpts
	} else {
		webhookOpts.CertDir = cfg.Controller.Webhook.CertDir
	}
